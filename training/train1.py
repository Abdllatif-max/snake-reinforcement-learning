...
SAVE_BEST_PATH = "best_dqn_snake.pth"  # File to store the best model

############################################
# 2) SnakeGame Environment
############################################
class SnakeGame:
    """
    Environment with:
    - 3 actions (left, straight, right) relative to current heading.
    - State of length 22:
        8 distances to snake body  (in 8 directions)
        8 distances to wall/bound (in 8 directions)
        4 binary inputs for food direction (up, down, left, right)
        2 inputs for current (dx, dy) direction
    """
    def __init__(self):
        ...
        # If direction = (dx, dy), then:
        #    left_turn[dx, dy]  -> next direction
        #    right_turn[dx, dy] -> next direction
        self.left_turn =  {
            (-1,0): (0,-1),  # up -> left
            (1,0):  (0,1),   # down -> right
            (0,-1): (1,0),   # left -> down
    ...
    def reset(self):
        ...
        # Start moving upward (dx=-1, dy=0) or any direction you prefer
        self.direction = (-1, 0)
    ...
    def _distance_to_wall(self, head_x, head_y, dx, dy):
        """
        Distance to the boundary in a given direction (dx, dy).
        We'll step outward until out of bounds.
        """
        distance = 0
        ...
        while 0 <= cur_x < GRID_SIZE and 0 <= cur_y < GRID_SIZE:
            ...
            cur_y += dy
            distance += 1
        return distance  # number of steps until out of grid

    def _distance_to_body(self, head_x, head_y, dx, dy):
        """
        Distance to the nearest body segment in direction (dx, dy).
        We'll step outward until we hit a segment of the snake body or go out of bounds.
        If we never collide, return some large or max distance possible.
        """
        distance = 0
        cur_x, cur_y = head_x, head_y
        while True:
            ...
            if not (0 <= cur_x < GRID_SIZE and 0 <= cur_y < GRID_SIZE):
                return distance  # No body found in-bounds
    ...
    def _get_food_direction(self, head_x, head_y):
        """
        Return 4 binary inputs: [food_up, food_down, food_left, food_right]
        E.g., if food_x < head_x => food_up=1, etc.
        """
        fx, fy = self.food
    ...
    def _get_state(self):
        """
        Returns a state vector of length 22:
          - 8 distances to body (in each of 8 directions)
          - 8 distances to wall (in each of 8 directions)
          - 4 binary food direction flags
          - 2 floats for current direction (dx, dy)
        """
        ...
        for (dx, dy) in self.directions_8:
            dist_body = self._distance_to_body(head_x, head_y, dx, dy)
            body_dists.append(float(dist_body))

        # Distances to wall
        wall_dists = []
        for (dx, dy) in self.directions_8:
            dist_wall = self._distance_to_wall(head_x, head_y, dx, dy)
            wall_dists.append(float(dist_wall))

        # Food direction
        food_dir = self._get_food_direction(head_x, head_y)

        # Current heading (dx, dy)
    ...
    def step(self, action):
        ...
        Direction updates happen accordingly.
        """
        if self.done:
        ...
        if new_head == self.food:
            ...
            self.snake.pop()
            reward = -0.05  # Small negative step cost
...
class DQN(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        # Bigger network: 3 hidden layers of 256 each, then 128, then out
...
############################################
# 4) DQN Agent with Soft Target Updates
############################################
class DQNAgent:
    ...
    def select_action(self, state):
        ...
        # Inference (eval) mode for batchnorm to use running stats
        self.q_network.eval()
    ...
    def train_step(self):
        ...
        # Switch to training mode (for batchnorm to handle multiple samples)
        ...
        # Next action from online network
...
if __name__ == "__main__":
    ...
    # We have 22 inputs: 8 body dists + 8 wall dists + 4 food direction + 2 direction
    STATE_DIM = 22
    # We want 3 actions: left, forward, right
    ACTION_DIM = 3
    ...
    print(f"💾 Best Model Saved: '{SAVE_BEST_PATH}'")
